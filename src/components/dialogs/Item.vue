<template>
    <v-layout row justify-center>
        <v-dialog v-model="showDialog" :fullscreen="shouldBeFullScreen" persistent
            :max-width="shouldBeFullScreen ? '' : '300px'">
            <v-card>
                ...
                <v-card-text>
                    <v-form v-model="validForm">
                        <v-combobox required label="Section *" v-model="section" :rules="notEmptyRule" :items="sections"
                            item-title="value" item-value="id">
                        </v-combobox>

                        <v-text-field required label="Item *" v-model="item.value" :rules="notEmptyRule"
                            placeholder="Enter the item you want to add">
                        </v-text-field>

                        <v-text-field label="Amount" v-model="item.amount" placeholder="Enter an amount">
                        </v-text-field>
                    </v-form>
                </v-card-text>
                ...
                <v-card-actions>
                    ...
                    <v-spacer></v-spacer>
                    <v-btn color="red" v-if="$route.params.sectionId || $route.params.itemId"
